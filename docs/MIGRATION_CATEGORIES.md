...
Ce document décrit comment passer des catégories « en dur » aux **catégories gérées dans Sanity**, afin de pouvoir créer, renommer et ordonner les catégories (Critiques / Enseignement) directement depuis le Studio.

## Comportement actuel

- Le site peut afficher les catégories définies dans le code (liste de secours) **ou** celles créées dans Sanity.
- Si des documents **Catégorie (Critiques / Enseignement)** existent dans Sanity, ils sont utilisés pour les cartes sur la page d’accueil (Critiques, Enseignement) et pour les libellés des pages.
...
2. Dans le menu de gauche, cliquez sur **Catégorie (Critiques / Enseignement)**.
...
Une fois ces documents créés, les cartes **Critiques** et **Enseignement** sur la page d’accueil utiliseront ces catégories (titres, descriptions, ordre). Les pages de liste (/critiques/…, /enseignement/…) utiliseront aussi ces libellés.
...
1. Studio → **Catégorie (Critiques / Enseignement)** → **Create new**.
2. Renseignez **Titre**, **Slug** (ex. `anatomie`), **Description**, **Section** (Critiques ou Enseignement), **Ordre d’affichage**.
